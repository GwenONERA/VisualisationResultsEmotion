...
# ═══════════════════════════════════════════════════════════════════════════════
# 3b. CORRECTION DES CHEVAUCHEMENTS POUR N=10
#     Pour N=10, les indices 4,5,6 forment le cluster HAUT (54°, 90°, 126°)
#     et les indices 0,1,9 forment le cluster BAS (-90°, -54°, 234°).
#     On décale les boîtes EXTÉRIEURES de chaque cluster (4,6 en haut / 1,9 en bas)
#     pour les éloigner de la boîte centrale (5 en haut / 0 en bas).
# ═══════════════════════════════════════════════════════════════════════════════
TOP_OUTER    = set()
BOTTOM_OUTER = set()
if MAX_TEXTS == 10 and N == 10:
    TOP_OUTER    = {4, 6}   # haut‑droite & haut‑gauche (exclut le centre 5)
    BOTTOM_OUTER = {1, 9}   # bas‑droite & bas‑gauche (exclut le centre 0)
...
for i, (_, row) in enumerate(data.iterrows()):
    ...
    # Correction originale (ex. "ça vous met en colere")
    if f"ça vous met en {EMOTION_WORD}" in row['sentence'].lower():
        sy -= 3.5

    # Correction anti‑chevauchement pour N=10
    if i in TOP_OUTER:
        sy -= 3.5          # descend les boîtes haut‑extérieures
    elif i in BOTTOM_OUTER:
        sy -= 3.5          # descend encore les boîtes bas‑extérieures (les pousse vers le bas)
